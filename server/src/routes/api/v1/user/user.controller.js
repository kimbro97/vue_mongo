...
controller.loginUser = async (req, res) => { // 로그인
    ...
    if(!userInfo) return res.status(400).json({ message: "이메일 정보를 확인해주세요" }) // 해당 이메일이 없다면
    ...
    const accessToken = signAccessToken({ _id, email, nickname, createdAt, updatedAt }) // access토큰을 만들어준다
    const refreshToken = signRefreshToken({ _id, email, nickname, createdAt, updatedAt }) // refresh토큰을 만들어준다

    if(!userInfo.refreshtoken){ // 로그인을 처음하거나 토큰값이 만료된다면
        userInfo.refreshtoken = refreshToken  //토큰값을 저장해준다
        await userInfo.save()
        return res.status(200).send({ success:true, accessToken, refreshToken }) // 그리고 access토큰을 보내준다
    }
    ...
    return res.status(200).send({ success:true, accessToken, refreshToken }) // 다 걸리지 않으면 accessToken을 다시 발급해준다
}
...
controller.deleteUser = async (req, res) => {
    ...
    await User.findOneAndDelete({_id: check._id})
    return res.status(200).send({success: true, message: "성공적으로 회원탈퇴 되었습니다"})
}
...
controller.updateUser = async (req, res) => {
    ...
    let userInfo = await User.findOne({_id: check._id})
    ...
    const accessToken = signAccessToken({ _id, nickname, createdAt, updatedAt })
    const refreshToken = signRefreshToken({ _id, nickname, createdAt, updatedAt })

    userInfo.refreshToken = refreshToken
    userInfo.save()

    return res.send({ success: true, userInfo: {_id, nickname, createdAt, updatedAt}, accessToken, refreshToken })
}
...
controller.getUserInfo = async (req, res) => {
    ...
    const { _id ,nickname, email, createdAt, updatedAt } = check

    return res.status(200).send({ success: true, userInfo: { _id, email, nickname, createdAt, updatedAt }})
}
